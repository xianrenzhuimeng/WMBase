...
- (instancetype)initWithGroupItems:(NSArray *)groupItems {
    ...
    _pager.height = 10;
    IPhoneXHeigh
    _pager.center = CGPointMake(self.width / 2, self.height - 18 - securityBottom_H);
...
- (void)showHUD:(NSString *)msg {
    ...
    label.font = font;
    label.text = msg;
    label.textColor = [TT_DarkmodeTool TT_NormalWhite];
...
- (UIButton *)baocun {
    if (!_baocun) {
        IPhoneXHeigh
        _baocun = [UIButton buttonWithType:UIButtonTypeCustom];
        _baocun.frame = CGRectMake([UIScreen mainScreen].bounds.size.width - 50, [UIScreen mainScreen].bounds.size.height - 40 - securityBottom_H, 40, 40);
        [_baocun setImage:[UIImage imageNamed:@"InviteCtrlSave"] forState:UIControlStateNormal];
        [_baocun addTarget:self action:@selector(longPress) forControlEvents:UIControlEventTouchUpInside];
    }
    return _baocun;
}
@end
